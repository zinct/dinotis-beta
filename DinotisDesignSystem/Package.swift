// swift-tools-version: 5.8
// The swift-tools-version declares the minimum version of Swift required to build this package.

import PackageDescription

import PackageDescription

let package = Package(
    name: "DinotisDesignSystem",
    defaultLocalization: "id",
    platforms: [.iOS(.v15)],
    products: [
        // Products define the executables and libraries a package produces, and make them visible to other packages.
        .library(
            name: "DinotisDesignSystem",
            targets: ["DinotisDesignSystem"]),
    ],
    dependencies: [
        // Dependencies declare other packages that this package depends on.
        .package(url: "https://github.com/SDWebImage/SDWebImageSwiftUI.git", from: "2.2.1"),
        .package(url: "https://github.com/SDWebImage/SDWebImage.git", from: "5.14.2"),
        .package(url: "https://github.com/SDWebImage/SDWebImageSVGCoder.git", from: "1.6.1"),
        .package(path: "../DinotisData"),
        .package(url: "https://github.com/airbnb/lottie-ios.git", from: "3.4.3")
    ],
    targets: [
        // Targets are the basic building blocks of a package. A target can define a module or a test suite.
        // Targets can depend on other targets in this package, and on products in packages this package depends on.
        .target(
            name: "DinotisDesignSystem",
            dependencies: [
                .byName(name: "SDWebImageSwiftUI"),
                .product(name: "SDWebImage", package: "SDWebImage"),
                .byName(name: "SDWebImageSVGCoder"),
                "DinotisData",
                .product(name: "Lottie", package: "lottie-ios")
            ],
            resources: [
                .process("./Font/FontStyle"),
                .process("./DinotisColor/Color"),
                .process("./DinotisAssets/Icon"),
                .process("./DinotisAssets/Illustration"),
                .process("./Localization/String"),
                .process("./DinotisAssets/Lottie")
            ]
        ),
        .testTarget(
            name: "DinotisDesignSystemTests",
            dependencies: ["DinotisDesignSystem", "DinotisData"]),
    ]
)
